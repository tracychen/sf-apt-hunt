...
export async function PUT(request: Request) {
  try {
    ...
    const body = putWorkspaceMapRequestSchema.parse(
    ...
    if (error instanceof RequestBodyTooLargeError) {
      return Response.json({ ok: false, error: "Workspace map request is too large." }, { status: 413 });
    }
    ...
    console.error("[workspace-map-route]", error);
    return Response.json({ ok: false, error: "Workspace map update failed." }, { status: 500 });
  }
}
